...
class Synthesizer:
    ...
    def my_synthesize(self, speaker_embeds, texts):
        ...
        linears, mels, alignments, stop_tokens = self.session.run(
            ...
            feed_dict=feed_dict)
        linears, mels, alignments, stop_tokens = list(linears[0]),list(mels[0]), alignments[0], stop_tokens[0]
        ...
        for i in range(len(mels)):
            try:
                ...
                linears[i] = linears[i][:target_length, :]
                
            except ValueError:
                # If no token is generated, we simply do not trim the output
                continue
        
        return [linear.T for linear in linears], [mel.T for mel in mels], alignments
